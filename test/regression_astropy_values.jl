...
@testset "HPX projection (Astropy comparison)" begin
    hdr = Dict(
        "NAXIS"  => 2,
        "CTYPE1" => "RA---HPX",  "CTYPE2" => "DEC--HPX",
        "CRPIX1" => 181.0,       "CRPIX2" => 91.0,
        "CRVAL1" => 0.0,         "CRVAL2" => 0.0,
        "CDELT1" => -1.0,        "CDELT2" => 1.0,
    )
    ...
    for (pix, world_ref) in [
        ...
        ([150.0,  91.0], [31.0, 0.0]),
        ([181.0, 110.0], [0.0, 16.3486440641493]),
        ([181.0,  70.0], [0.0, -18.12620396055673]),
        ([50.0,   91.0], [131.0, 0.0]),
        ([300.0,  91.0], [241.0, 0.0]),
        ([100.0,  50.0], [81.0, -37.402276799166195]),
        ([250.0, 130.0], [291.0, 35.29439467666014]),
        # Polar regime (|sin(theta)| > 2/3).
        ([250.0, 140.0], [288.6585365854, 46.3269622294]),
        ([100.0, 140.0], [84.5121951220, 46.3269622294]),
        ([250.0,  40.0], [287.3076923077, -48.5583054466]),
...
@testset "XPH projection (Astropy comparison)" begin
    hdr = Dict(
        "NAXIS"  => 2,
        "CTYPE1" => "RA---XPH",  "CTYPE2" => "DEC--XPH",
        "CRPIX1" => 181.0,       "CRPIX2" => 91.0,
        "CRVAL1" => 0.0,         "CRVAL2" => 0.0,
        "CDELT1" => -1.0,        "CDELT2" => 1.0,
    )
    ...
    for (pix, world_ref) in [
        ...
        ([150.0,  91.0], [22.941201187547048, 4.598371040040415e-15]),
        ([181.0, 110.0], [0.0, 14.001814149876111]),
        ([181.0,  70.0], [1.9438336296687695e-15, -15.484287359968391]),
        ([250.0, 130.0], [282.69975087562887, 23.267410439502523]),
        ([181.0, 140.0], [0.0, 36.64136374577015]),
        ([181.0,  40.0], [5.520337456119047e-15, -38.19360363495788]),
    ]
        world = pixel_to_world(wcs, pix)
        @test world ≈ world_ref atol=1e-10
        @test world_to_pixel(wcs, world_ref) ≈ pix atol=1e-8
...
@testset "ZPN cubic bisection (Astropy comparison)" begin
    hdr = Dict(
        "NAXIS"  => 2,
        "CTYPE1" => "RA---ZPN",  "CTYPE2" => "DEC--ZPN",
        "CRPIX1" => 128.0,       "CRPIX2" => 96.0,
    ...
    for (pix, world_ref) in [
        ...
        ([128.0, 110.0], [120.0, 35.69914583349805]),
        ([100.0,  80.0], [121.68778355479681, 34.19052949849114]),
        ([160.0, 120.0], [118.02466161512633, 36.17990495626172]),
    ]
        world = pixel_to_world(wcs, pix)
        @test world ≈ world_ref atol=1e-10
        @test world_to_pixel(wcs, world_ref) ≈ pix atol=1e-8
...
@testset "CYP non-default parameters (Astropy comparison)" begin
    hdr = Dict(
        "NAXIS"  => 2,
        "CTYPE1" => "RA---CYP",  "CTYPE2" => "DEC--CYP",
        "CRPIX1" => 128.0,       "CRPIX2" => 96.0,
        "CRVAL1" => 120.0,       "CRVAL2" => 35.0,
        "CDELT1" => -0.05,       "CDELT2" => 0.05,
    ...
    for (pix, world_ref) in [
        ...
        ([128.0, 110.0], [120.0, 35.544438592121786]),
        ([100.0,  80.0], [121.13074279277149, 34.372504075843366]),
        ([160.0, 120.0], [118.68289274189692, 35.9262717977907]),
    ]
        world = pixel_to_world(wcs, pix)
        @test world ≈ world_ref atol=1e-10
        @test world_to_pixel(wcs, world_ref) ≈ pix atol=1e-8
...
@testset "Conic second parameter set (Astropy comparison)" begin
    for (code, sigma, delta, refs) in [
        ("COP", 70.0, 10.0, [
            ([128.0,  96.0], [120.0, 35.00000000000001]),
            ([140.0,  96.0], [119.2562495433403, 34.99773180876521]),
            ([128.0, 110.0], [120.0, 35.71076216693829]),
            ([100.0,  80.0], [121.71821579324889, 34.17549468682815]),
            ([160.0, 120.0], [117.98690789514018, 36.20198232804094]),
        ]),
        ("COD", 70.0, 10.0, [
            ([128.0,  96.0], [120.0, 35.00000000000001]),
            ([140.0,  96.0], [119.25625217536329, 34.997911385218366]),
            ([128.0, 110.0], [120.0, 35.699999999999996]),
            ([100.0,  80.0], [121.71779477866362, 34.18870143007145]),
            ([160.0, 120.0], [117.98617955206237, 36.18500282578722]),
        ]),
        ("COE", 70.0, 10.0, [
            ([128.0,  96.0], [120.0, 35.00000000000001]),
            ([140.0,  96.0], [119.25692048249506, 34.99811927326666]),
            ([128.0, 110.0], [120.0, 35.68984082302652]),
            ([100.0,  80.0], [121.71629438951517, 34.20098777477994]),
            ([160.0, 120.0], [117.98803050059954, 36.169050002218356]),
        ]),
        ("COO", 70.0, 10.0, [
            ([128.0,  96.0], [120.0, 35.00000000000001]),
            ([140.0,  96.0], [119.25599874364043, 34.997681258555076]),
            ([128.0, 110.0], [120.0, 35.71108840998695]),
            ([100.0,  80.0], [121.71842171701148, 34.175013667850756]),
            ([160.0, 120.0], [117.98557131639971, 36.20227930386141]),
        ]),
    ]
        hdr = Dict(
            "NAXIS"  => 2,
